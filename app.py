...
class HowdiesBot:
    # <-- BADLAV YAHAN: initial_room ki jagah initial_rooms (list) le rahe hain
    def __init__(self, username, password, initial_rooms):
        # Bot configuration
        self.BOT_MASTER = username
        self.USERNAME = username
        self.PASSWORD = password
        self.INITIAL_ROOMS = initial_rooms # <-- BADLAV YAHAN
    ...
    # <-- BADLAV YAHAN: Panel se room join karne ke liye naya function
    def join_room_from_panel(self, room_name, password=""):
        """Joins a new room while the bot is running, triggered from the panel."""
        if not room_name:
            ...
            try:
                ...
                while self.should_run:
                    ...
                    if handler == "login" and data.get("status") == "success":
                        ...
                        time.sleep(1)
                        # <-- BADLAV YAHAN: Ab ek-ek karke saare default rooms join karega
                        ...
                        for room in self.INITIAL_ROOMS:
                            ...
                            time.sleep(random.uniform(1, 2)) # Har join ke beech thoda gap
...
# <-- BADLAV YAHAN: HTML template mein naya form add kiya gaya hai
HTML_TEMPLATE = """
<!DOCTYPE html>
<html lang="en">
<head>
...
def start_bot():
    ...
    with status_lock:
        ...
        # <-- BADLAV YAHAN: Default rooms ki list yahan define ki gayi hai
        default_rooms = ["smile", "gujarat", "dragon", "london", "news"]

        bot_status["username"] = username
        bot_status["room"] = ", ".join(default_rooms) # Display ke liye
        bot_status["log"] = ["INFO: Bot ko start karne ka request mila..."]
        
        # <-- BADLAV YAHAN: Bot ko default rooms ki list di ja rahi hai
...
# <-- BADLAV YAHAN: Panel se room join karne ke liye naya route
@app.route('/join_room', methods=['POST'])
def join_room_live():
    """Tells the running bot to join a new room."""
    with status_lock:

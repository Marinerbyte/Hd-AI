...
class Config:
    ...
    # --- ARSENAL CLASH GAME CONFIG (UPGRADED) ---
    ARSENAL_MAP = {
        "1": "Bomb 💣", "2": "Fire 🔥", "3": "Rocket 🚀",
        "4": "Explosion 💥", "5": "Gun 🔫", "6": "Knife 🔪"
    }
    CHALLENGE_EXPIRY_SECONDS = 60 # NEW: Duel challenge expires if not accepted
    TURN_TIMEOUT_SECONDS = 60     # NEW: Player forfeits turn if inactive
    XP_WIN = 10
    XP_LOSS = 1
    XP_TIE = 5
...
# --- ARSENAL CLASH GAME STATE (UPGRADED) ---
game_state_lock = threading.Lock()  # NEW: Lock to prevent race conditions
...
# ========================================================================================
# === 8. ARSENAL CLASH GAME LOGIC (UPGRADED V2) ==========================================
# ========================================================================================
def broadcast_to_duel(duel_id, message):
    # This function is now safer because it checks if the game still exists inside the lock
...
def advance_turn(duel_id):
    with game_state_lock:
        ...
        # Delay message sending slightly after phase change to prevent message flood
        if phase_change:
            time.sleep(1)

        reply_to_room(attacker['room_id'], f"Turn {game_state['current_turn']}/6. Your remaining arsenal:\n{arsenal_list}\nChoose your attack by typing a number. You have {Config.TURN_TIMEOUT_SECONDS} seconds.")
        
        # Start timer for the new attacker's turn
        game_state["turn_timer"] = threading.Timer(Config.TURN_TIMEOUT_SECONDS, handle_turn_timeout, args=[duel_id])
        game_state["turn_timer"].start()

def handle_turn_timeout(duel_id):
    with game_state_lock:
        if duel_id not in active_duels: return
        game_state = active_duels[duel_id]
        
        # Check whose turn it was when timeout happened
        is_attacker_turn = game_state["current_attack_weapon"] is None
        
    if is_attacker_turn:
        # Attacker failed to choose a weapon
        attacker_key = game_state["attacker_key"]
        attacker_name = game_state["players"][attacker_key]["username"]
        defender_name = game_state["players"]["p2" if attacker_key == "p1" else "p1"]["username"]
        reason_msg = f"⏰ **TIMEOUT!** `{attacker_name}` failed to attack in time. **`{defender_name}` wins by forfeit!**"
        end_game(duel_id, reason="forfeit", forfeiter_key=attacker_key, custom_reason_msg=reason_msg)
    else:
        ...
        with game_state_lock:
             if duel_id not in active_duels: return # Check again in case game ended
...
def handle_game_turn(sender, room_id, move):
    ...
    with game_state_lock:
        if duel_id not in active_duels: return # Game might have ended
        ...
        if sender_name_lower == game_state["players"][attacker_key]["username"].lower():
            ...
            if move not in game_state["remaining_arsenals"][attacker_key]:
                reply_to_room(room_id, f"You've already used that weapon or it's an invalid choice. Try again.")
                return
            
            # Cancel attacker's turn timer
            if game_state.get("turn_timer"): game_state["turn_timer"].cancel()
            ...
            defense_prompt = (
                ...
                f"Guess their weapon by typing a number. You have {Config.TURN_TIMEOUT_SECONDS} seconds!\n\n"
        ...
        # --- DEFENDER'S TURN ---
            ...
            # Directly call advance_turn. It's now safe.
            advance_turn(duel_id)

def expire_duel_challenge(target_room_id, challenger_room_id):
    with game_state_lock:
        if target_room_id not in pending_duels: return # Already accepted or cancelled
        
        challenger_info = pending_duels.pop(target_room_id)
        challenger_name = challenger_info["challenger_name"]
        
    challenger_room_name = bot_state.room_id_to_name.get(challenger_room_id, "An unknown room")
    target_room_name = bot_state.room_id_to_name.get(target_room_id, "this room")
    
    logging.info(f"Duel challenge from {challenger_name} to room {target_room_name} expired.")
    reply_to_room(challenger_room_id, f"⏰ Your duel challenge to room '{target_room_name}' has expired.")
    reply_to_room(target_room_id, f"⏰ The duel challenge from `{challenger_name}` in room '{challenger_room_name}' has expired.")
...
def handle_game_command(sender, room_id, command, args):
    ...
    with game_state_lock:
        if command == "duel":
            ...
            for challenge in pending_duels.values():
                if challenge['challenger_room_id'] == room_id:
                    return reply_to_room(room_id, "You already have a pending challenge out. Use `!cancelduel` first.")

            expiry_timer = threading.Timer(Config.CHALLENGE_EXPIRY_SECONDS, expire_duel_challenge, args=[target_room_id, room_id])
            expiry_timer.start()
            
            pending_duels[target_room_id] = {
                "challenger_room_id": room_id, 
                "challenger_name": sender_name,
                "expiry_timer": expiry_timer
            }
            reply_to_room(room_id, f"Challenge sent to '{target_room_name}'. They have {Config.CHALLENGE_EXPIRY_SECONDS} seconds to accept.")
            challenger_room_name = bot_state.room_id_to_name.get(room_id, "An unknown room")
            reply_to_room(target_room_id, f"⚔️ **CHALLENGE!** ⚔️\n`{sender_name}` from room **{challenger_room_name}** has challenged you to an Arsenal Clash!\nType `!acceptduel` to begin! This challenge will expire in {Config.CHALLENGE_EXPIRY_SECONDS} seconds.")
            ...
            challenger_info['expiry_timer'].cancel()
            ...
            active_duels[duel_id] = {
                ...
                "duel_id": duel_id, "scores": {"p1": 0, "p2": 0}, "phase": "p1_attack",
                "current_turn": 0, "attacker_key": "p1", "defender_key": "p2",
                "remaining_arsenals": {"p1": all_weapons.copy(), "p2": all_weapons.copy()},
                "current_attack_weapon": None, "turn_timer": None
            }
            ...
            challenge_to_cancel = None
            for target_id, info in pending_duels.items():
                if info['challenger_room_id'] == room_id:
                    challenge_to_cancel = target_id
                    break
            
            if not challenge_to_cancel:
                return reply_to_room(room_id, "You don't have any pending challenges to cancel.")

            info = pending_duels.pop(challenge_to_cancel)
            info['expiry_timer'].cancel()
            
            target_room_name = bot_state.room_id_to_name.get(challenge_to_cancel, "the other room")
            reply_to_room(room_id, "Your duel challenge has been cancelled.")
            reply_to_room(challenge_to_cancel, f"The duel challenge from room '{bot_state.room_id_to_name.get(room_id)}' has been cancelled.")
...
def process_command(sender, room_id, message_text):
    ...
    game_commands = ["duel", "acceptduel", "forfeit", "rank", "leaderboard", "cancelduel"]
    if command in game_commands:
        ...
        if command in ["rank", "leaderboard"]:
            ...
            # Game commands are now thread-safe due to the lock
            handle_game_command(sender, room_id, command, args)
        return
    ...
    if command in ["gamehelp", "duelhelp"]:
        ...
        reply_to_room(room_id, "⚔️ **How to Play: Arsenal Clash** ⚔️\n\n"
                               ...
                               "**1. Challenge:** `!duel <room_name>` (Expires in 60s)\n"
                               "**2. Accept:** `!acceptduel`\n"
                               "**3. Cancel:** `!cancelduel` (To cancel a challenge you sent)\n\n"
                               "**Gameplay:**\n"
                               "- **To Attack/Defend:** When it's your turn, simply type the number of the weapon you want to use (e.g., `3`).\n"
                               "- **Timeout:** You have 60 seconds to make your move, or you will forfeit the turn/game!\n\n"
                               "**Rules:**\n"
                               "- Each player attacks 6 times.\n"
                               "- A correct defense is a **BLOCK**. An incorrect one is a **HIT**.\n\n"
                               "**Weapons:**\n"
                               f"{weapon_list}\n\n"
    ...
    try:
        ...
        if handler == "login" and data.get("status") == "success":
            ...
            if room_id in busy_rooms and message_text.isdigit() and message_text in Config.ARSENAL_MAP:
                handle_game_turn(sender, room_id, message_text)
                return
...
def connect_to_howdies():
    ...
    with game_state_lock:
        ...
        # Cancel any running timers before clearing
        for duel in active_duels.values():
            if duel.get("turn_timer"): duel["turn_timer"].cancel()
        for challenge in pending_duels.values():
            if challenge.get("expiry_timer"): challenge["expiry_timer"].cancel()

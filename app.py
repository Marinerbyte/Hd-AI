...
def start_bot_logic():
    ...
    if not bot_thread or not bot_thread.is_alive():
        ...
        bot_state = BotState()
        load_masters() # <-- FIX #1: RELOAD MASTERS ON RESTART
...
def handle_master_command(sender, command, args, room_id):
    try:
        ...
        # <-- FIX #2: !memories command logic changed to prevent crash
        elif command == 'memories':
            if not args or not args[0].startswith('@'):
                return reply_to_room(room_id, f"@{sender['name']} Usage: `!memories @username`")
            ...
            try:
                ...
                # Bhejne ke liye message taiyar karna
                if facts_res.data and facts_res.data[0]['facts']:
                    memory_report = f"@{sender['name']} Here are the memories for **{target_user}**:\n\n{facts_res.data[0]['facts']}"
                else:
                    memory_report = f"@{sender['name']} I don't have any long-term memories for **{target_user}** yet."
                
                # PM ke bajaye, seedha room mein message bhejna (yeh crash nahi hoga)
                reply_to_room(room_id, memory_report)

            except Exception as e:
                logging.error(f"Error fetching memories for {target_user}: {e}")
                reply_to_room(room_id, f"@{sender['name']} Sorry, I couldn't fetch the memories. My database might be busy.")
...
# ========================================================================================
# === MAIN EXECUTION BLOCK (MODIFIED FOR DEPLOYMENT) =====================================
# ========================================================================================
setup_logging()
load_masters()
# initialize_database() # <-- Yeh line yahan se hata di gayi hai.

if __name__ == "__main__":
    # Database sync ko ek alag background thread me shuru karein
    # taaki web server turant start ho jaye aur Render ka health check pass ho.
    logging.info("--- Starting Database Sync in background... ---")
    db_init_thread = threading.Thread(target=initialize_database, daemon=True)
    db_init_thread.start()

    # Ab web server ko start karein

...
def format_emojis(text):
    """
    Finds all emojis in a string and wraps them like _emoji_ for a cuter look.
    """
    # A comprehensive regex to find most emoji characters
    ...
    # The replacement function adds spaces and underscores around the found emoji
...
def get_ai_response(user_message, sender, room_id):
    ...
    try:
        ...
        # Remove any markdown like *word*
        ai_reply = re.sub(r'\*.*?\*', '', ai_reply).strip()

        # --- MODIFICACIÓN: Formatear emojis para que se vean más lindos ---
        ai_reply = format_emojis(ai_reply)

        # Update conversation history with the modified reply
        conversation_history.append({"role": "assistant", "content": ai_reply})
        supabase.table('conversation_memory').upsert({'username': sender_lower, 'history': conversation_history}).execute()

        # --- MODIFICACIÓN: Aplicar siempre la fuente de mayúsculas pequeñas ---
        final_reply = to_small_caps(ai_reply)
        
        # Clean up any extra spaces created by emoji formatting

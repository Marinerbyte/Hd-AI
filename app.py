...
class HowdiesBot:
    def __init__(self, username, password, initial_rooms):
        ...
        self.master_report_room_id = None 
    ...
    # <-- BADLAV YAHAN: Payload update kar diya gaya hai
    def visit_user_profile(self, username):
        ...
        # Dusre bot se study kiya gaya asli payload
        ...
        # Flood se bachne ke liye random delay
        time.sleep(random.uniform(1.5, 4.0))

    def scan_all_rooms_and_report(self):
        """
        This function runs in a background thread to scan all rooms.
        """
        self.log("INFO: Profile scanner thread shuru ho gaya hai. Rooms join hone ka intezar...")
        time.sleep(15) 

        if not self.master_report_room_id:
            self.log("ERROR: Master ko report karne ke liye koi room ID set nahi hai. Scanner band ho raha hai.")
            return
        ...
        for room_id, room_data in rooms_to_scan:
            ...
            # Bot ko khud ki profile visit nahi karni hai
            ...
            self.send_chat_message(self.master_report_room_id, report_message)
            self.log(f"SCANNER: Room '{room_name}' ki report master ko bhej di gayi.")
            time.sleep(5) 
            
        self.log("INFO: Sabhi rooms ki profile scanning poori ho gayi hai.")
        self.send_chat_message(self.master_report_room_id, "🎉 All room scans finished.")
            ...
            try:
                ...
                while self.should_run:
                    ...
                    try:
                        ...
                        # Hum sirf zaroori messages hi log karenge taaki log spam na ho
                    ...
                    if handler == "login" and data.get("status") == "success":
                        ...
                        if room_id and name:
                            with self.room_data_lock:
                                ...
                                if self.master_report_room_id is None:
                                    self.master_report_room_id = room_id
                                    self.log(f"INFO: Master ko reports '{name}' room me bheji jayengi.")

...
class Config:
    ...
    ### --- MODIFIED START --- ###
    # MASTERS_LIST from .env is now removed. We use a database-driven system.
    ...
    MEMORY_LIMIT = 20 # Increased short-term memory
...
def initialize_database():
    ...
    # Universal brevity jailbreak to be added to all prompts
...
def load_masters():
    """
    Loads masters from the database and adds the hardcoded master.
    This function populates bot_state.masters.
    """
    if not supabase:
        logging.error("🔴 Supabase not available. Cannot load masters from database.")
        # Fallback to only the hardcoded master
    ...
    try:
        # Start with a set containing the hardcoded master to handle uniqueness
        masters_set = {Config.HARDCODED_MASTER}
        
        # Fetch masters from the Supabase table
        ...
        # Store the final list in the bot state
        ...
        bot_state.masters = [Config.HARDCODED_MASTER]
        logging.error(f"🔴 Supabase error loading masters: {e.message}")
        logging.warning("⚠️ Please ensure a 'masters' table with a 'username' column exists in Supabase.")
        logging.warning(f"⚠️ Running with only the hardcoded master: {Config.HARDCODED_MASTER}")
    except Exception as e:
        bot_state.masters = [Config.HARDCODED_MASTER]
        logging.error(f"🔴 A general error occurred while loading masters: {e}")
        logging.warning(f"⚠️ Running with only the hardcoded master: {Config.HARDCODED_MASTER}")
### --- MODIFIED END --- ###
...
def update_user_facts(username, conversation_history):
    """
    Summarizes the conversation and updates the long-term memory ('fact sheet') for a user.
    This runs in a background thread to not delay the bot's response.
    """
    ...
    try:
        ...
        # 2. Create a prompt for the summarizer AI
        summarizer_prompt = (
        ...
        # 4. Update the database if new facts were found
...
def get_ai_response(user_message, sender, room_id):
    ...
    try:
        # 1. Fetch Long-Term Memory (Fact Sheet)
        ...
        if behavior_response.data:
            ...
            # Priority 2: Room-specific personality -> Fallback to default
        ...
        # 4. Fetch Short-Term Memory (Recent Conversation)
        ...
        # 5. Call Groq for the main response
...
def handle_master_command(sender, command, args, room_id):
    try:
        # --- NEW MASTER MANAGEMENT COMMANDS ---
        if command == 'am': # Add Master
            ...
            bot_state.masters.append(target_user) # Update in-memory list
            reply_to_room(room_id, f"✅ Done. `{target_user}` is now a master.")

        elif command == 'dm': # Delete Master
            ...
            bot_state.masters.remove(target_user) # Update in-memory list
            ...
            if db_masters:
                reply += f"`{', '.join(db_masters)}`"
            else:
                reply += "_None_"
            reply_to_room(room_id, reply)

        # --- EXISTING MASTER COMMANDS ---
        elif command == 'adb':
...
def process_command(sender, room_id, message_text):
    ...
    if command == 'help':
        ...
        # Expanded the list of recognized master commands

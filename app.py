...
class HowdiesBot:
    ...
    def scan_all_rooms_and_report(self, initial_wait=True):
        """Scans all rooms and sends reports directly to the master via DM."""
        # Yeh function ab sirf scan karta hai, intezaar nahi.
        with self.room_data_lock:
    ...
    def post_login_tasks(self):
        """Login ke baad ke saare kaam yahan honge: rooms join karna aur phir scan shuru karna."""
        self.log("INFO: Login ke baad ka process shuru. Pehle rooms join honge.")
        
        # 1. Aaram se ek-ek karke sabhi rooms ko join karne ka request bhejo
        for room in self.INITIAL_ROOMS:
            self.log(f"ATTEMPT: Room '{room}' ko join karne ka request bhej raha hai.")
            self.send_payload({"handler": "joinchatroom", "name": room, "roomPassword": ""})
            time.sleep(random.uniform(2, 4)) # Har join ke beech aaram se gap

        # 2. Server ko response dene ke liye 30 second ka grace period do
        self.log("INFO: Sabhi join requests bhej di gayi hain. 30 second ka grace period shuru...")
        time.sleep(30)
        
        self.log("INFO: Grace period khatam. Ab profile visit ka kaam shuru hoga.")
        
        # 3. Ab profile visit shuru karo
            ...
            try:
                ...
                while self.should_run:
                    ...
                    if handler == "login" and data.get("status") == "success":
                        ...
                        # Sirf ek thread shuru karo jo saare kaam aaram se karega
                        task_thread = threading.Thread(target=self.post_login_tasks)

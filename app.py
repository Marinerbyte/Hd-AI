...
def load_masters():
    ...
    if masters_str:
        bot_state.masters = [name.strip().lower() for name in masters_str.split(',') if name.strip()]
...
def join_startup_rooms():
    ...
    if not rooms_str:
        ...
        return
    room_names = [name.strip() for name in rooms_str.split(',') if name.strip()]
...
def update_user_facts(username, conversation_history):
    ...
    try:
        logging.info(f"🧠 Updating and summarizing long-term memory for {username}...")
        
        facts_res = supabase.table('user_facts').select('facts').eq('username', username).execute()
        existing_facts = facts_res.data[0]['facts'] if facts_res.data and facts_res.data[0]['facts'] else "No facts known yet."

        summarizer_prompt = (
            "You are a meticulous fact consolidation AI. Your task is to analyze an existing list of facts about a user and a recent conversation snippet. "
            "Your goal is to produce a single, clean, de-duplicated, and concise list of key facts. \n\n"
            "## INSTRUCTIONS:\n"
            "1.  **Integrate New Information:** Add new, important facts from the conversation to the list.\n"
            "2.  **De-duplicate:** If a fact is already known, do not repeat it.\n"
            "3.  **Summarize & Refine:** Combine related facts. For example, if you know 'user likes cats' and the conversation mentions 'user loves kittens', update it to 'user loves cats and kittens'.\n"
            "4.  **Be Concise:** Keep the facts brief and to the point. Use bullet points (e.g., `- Fact 1`).\n"
            "5.  **Output ONLY the final, clean list of facts.** Do not add any extra commentary like 'Here is the updated list'.\n\n"
            f"## Existing Facts about {username}:\n{existing_facts}\n\n"
            f"## New Conversation to Analyze:\n{json.dumps(conversation_history[-4:])}\n\n"
            f"## Final Consolidated Fact Sheet for {username}:"
        )
        
        messages = [{"role": "system", "content": summarizer_prompt}]
        headers = {"Authorization": f"Bearer {Config.GROQ_API_KEY}", "Content-Type": "application/json"}
        payload = {"model": "llama3-8b-8192", "messages": messages, "temperature": 0.2, "max_tokens": 500} 
        
        api_response = requests.post(Config.GROQ_API_URL, headers=headers, json=payload, timeout=20)
        api_response.raise_for_status()
        
        cleaned_facts = api_response.json()['choices'][0]['message']['content'].strip()
        
        if cleaned_facts and cleaned_facts.lower() != existing_facts.lower():
            supabase.table('user_facts').upsert({'username': username, 'facts': cleaned_facts}).execute()
            logging.info(f"🧠✅ Long-term memory for {username} has been successfully summarized and updated.")
        else:
            logging.info(f"🧠 No new significant facts learned for {username}. Memory remains unchanged.")
    ...
    try:
        if command == 'adb':
            ...
            try:
                ...
                # --- CHUNKING LOGIC TO PREVENT DISCONNECTS ---
                CHUNK_SIZE = 450  # Safe character limit for a single message

                if len(memory_report) <= CHUNK_SIZE:
                    send_pm(sender['id'], memory_report)
                else:
                    chunks = [memory_report[i:i + CHUNK_SIZE] for i in range(0, len(memory_report), CHUNK_SIZE)]
                    total_parts = len(chunks)
                    for i, chunk in enumerate(chunks):
                        part_message = f"Part {i+1}/{total_parts}:\n{chunk}"
                        send_pm(sender['id'], part_message)
                        time.sleep(1) # Delay between each part to avoid flooding
                
                time.sleep(1) # Delay before sending confirmation to the room
...
def process_command(sender, room_id, message_text):
    ...
    if command == 'help':
        ...
        if command in ['pers', 'addpers', 'delpers', 'listpers', 'adb', 'rmb', 'status', 'memories']:
...
def on_message(ws, message_str):
    ...
    try:
        ...
        if handler == "login" and data.get("status") == "success":
            ...
            startup_rooms = [name.strip().lower() for name in Config.ROOMS_TO_JOIN.split(',') if name.strip()]

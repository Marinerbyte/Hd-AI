...
class Config:
    ...
    # --- ARSENAL CLASH GAME CONFIG ---
    ARSENAL = {"💣", "🔥", "🚀", "💥", "🔫", "🔪"}
    DEFEND_TIME_LIMIT_SECONDS = 15
    XP_WIN = 10
    XP_LOSS = 1
    XP_TIE = 5
...
# ========================================================================================
# === 8. ARSENAL CLASH GAME LOGIC ========================================================
# ========================================================================================
def broadcast_to_duel(duel_id, message):
    """Sends a message to both rooms involved in a duel."""
    if duel_id not in active_duels: return
    game_state = active_duels[duel_id]
    room1_id = game_state["players"]["p1"]["room_id"]
    room2_id = game_state["players"]["p2"]["room_id"]
    reply_to_room(room1_id, message)
    if room1_id != room2_id:
        reply_to_room(room2_id, message)

def get_duel_id(room_id1, room_id2):
    """Creates a consistent, sorted tuple to use as a dictionary key."""
    return tuple(sorted((room_id1, room_id2)))

def get_game_from_room(room_id):
    """Finds the active duel a room is participating in."""
...
def update_player_stats_in_db(winner_name, loser_name, is_tie=False):
    """Updates player stats in Supabase using a remote procedure call."""
...
def end_game(duel_id, reason="completed", forfeiter_key=None):
    """Cleans up a finished game, announces the winner, and updates stats."""
    if duel_id not in active_duels: return
    game_state = active_duels.pop(duel_id) # Remove from active games immediately
    ...
    if reason == "forfeit" and forfeiter_key:
    ...
    # Use the original duel_id from the state, as it's a tuple of room IDs
...
def advance_turn(duel_id):
    """Moves the game to the next turn or phase."""
    if duel_id not in active_duels: return
    game_state = active_duels[duel_id]
    ...
    # Check if the current attacker's phase is over
    if not game_state["remaining_arsenals"][game_state["attacker_key"]]:
        ...
        if game_state["phase"] == "p1_attack":
            ...
            new_attacker_name = game_state["players"]["p2"]["username"]
            broadcast_to_duel(duel_id, f"PHASE END! It's now `{new_attacker_name}`'s turn to attack!")
        else: # p2's attack phase just ended
            end_game(duel_id, "completed")
            return
    else:
        # Just advance the turn number
        game_state["current_turn"] += 1
    
    attacker_name = game_state["players"][game_state["attacker_key"]]["username"]
    arsenal = ' '.join(game_state['remaining_arsenals'][game_state['attacker_key']])
    broadcast_to_duel(duel_id, f"Turn {game_state['current_turn']}/6. `{attacker_name}`, choose your next attack. Remaining: `{arsenal}`")

def handle_timeout(duel_id):
    """Handles the defender running out of time."""
    if duel_id not in active_duels: return
    ...
    broadcast_to_duel(duel_id, f"⏰ **TIMEOUT!** `{defender_name}` failed to defend.\n**HIT!** `{attacker_name}` scored with {weapon_used}.")

    if game_state["players"][defender_key]["warnings"] >= 2:
        broadcast_to_duel(duel_id, f"🛑 `{defender_name}` has timed out twice!")
        end_game(duel_id, reason="forfeit", forfeiter_key=defender_key)
    else:
        advance_turn(duel_id)


def handle_game_command(sender, room_id, command, args):
    """Master router for all game-related commands."""
    ...
    if command == "duel":
        ...
        active_duels[duel_id] = { "players": { "p1": {"username": challenger_info["challenger_name"], "room_id": challenger_room_id, "warnings": 0}, "p2": {"username": sender_name, "room_id": room_id, "warnings": 0} }, "duel_id": duel_id, "scores": {"p1": 0, "p2": 0}, "phase": "p1_attack", "current_turn": 1, "attacker_key": "p1", "defender_key": "p2", "remaining_arsenals": {"p1": set(Config.ARSENAL), "p2": set(Config.ARSENAL)}, "current_attack_weapon": None, "defense_timer": None }
        
        p1_name = active_duels[duel_id]["players"]["p1"]["username"]
        p2_name = active_duels[duel_id]["players"]["p2"]["username"]
        
        broadcast_to_duel(duel_id, f"🔥 **DUEL ACCEPTED!** 🔥\n`{p1_name}` vs `{p2_name}`\nLet the **Arsenal Clash** begin!")
        time.sleep(1)
        arsenal = ' '.join(active_duels[duel_id]['remaining_arsenals']['p1'])
        broadcast_to_duel(duel_id, f"Turn 1/6. `{p1_name}`, you're attacking first! Choose your weapon with `!attack <emoji>`. Your arsenal: `{arsenal}`")

    else:
        game_state = get_game_from_room(room_id)
        if not game_state:
            if command in ["attack", "defend", "forfeit"]:
                reply_to_room(room_id, "This room isn't in a duel right now.")
            return

        duel_id, attacker_key, defender_key = game_state["duel_id"], game_state["attacker_key"], game_state["defender_key"]

        if command == "attack":
            if game_state["players"][attacker_key]["username"].lower() != sender_name_lower: return reply_to_room(room_id, "It's not your turn to attack.")
            if game_state["current_attack_weapon"] is not None: return reply_to_room(room_id, "An attack is already in progress. Wait for the defender.")
            if not args or args[0] not in Config.ARSENAL: return reply_to_room(room_id, f"Invalid weapon. Use one of: {' '.join(Config.ARSENAL)}")
            
            weapon = args[0]
            if weapon not in game_state["remaining_arsenals"][attacker_key]: return reply_to_room(room_id, f"You've already used {weapon}.")
                
            game_state["remaining_arsenals"][attacker_key].remove(weapon)
            game_state["current_attack_weapon"] = weapon
            ...
            defender_room_id = game_state["players"][defender_key]["room_id"]
            defender_name = game_state["players"][defender_key]["username"]
            reply_to_room(room_id, f"You have attacked with {weapon}. Waiting for `{defender_name}` to defend...")
            reply_to_room(defender_room_id, f"INCOMING ATTACK! `{defender_name}`, defend with `!defend <emoji>`. You have {Config.DEFEND_TIME_LIMIT_SECONDS} seconds!")
        
        elif command == "defend":
            if game_state["players"][defender_key]["username"].lower() != sender_name_lower: return reply_to_room(room_id, "It's not your turn to defend.")
            if game_state["current_attack_weapon"] is None: return reply_to_room(room_id, "There's no attack to defend against right now.")
            if not args or args[0] not in Config.ARSENAL: return reply_to_room(room_id, f"Invalid weapon. Use one of: {' '.join(Config.ARSENAL)}")
            
            game_state["defense_timer"].cancel()
            guess, actual_weapon = args[0], game_state["current_attack_weapon"]
            
            if guess == actual_weapon:
                broadcast_to_duel(duel_id, f"🛡️ **BLOCK!** `{sender_name}` correctly defended the {actual_weapon} attack!")
            else:
                game_state["scores"][attacker_key] += 1
                attacker_name = game_state["players"][attacker_key]["username"]
                broadcast_to_duel(duel_id, f"💥 **HIT!** `{sender_name}` guessed {guess}, but the attack was {actual_weapon}! Point to `{attacker_name}`.")
            advance_turn(duel_id)

        elif command == "forfeit":
            if sender_name_lower == game_state["players"]["p1"]["username"].lower(): end_game(duel_id, reason="forfeit", forfeiter_key="p1")
            elif sender_name_lower == game_state["players"]["p2"]["username"].lower(): end_game(duel_id, reason="forfeit", forfeiter_key="p2")
...
def process_command(sender, room_id, message_text):
    ...
    game_commands = ["duel", "acceptduel", "attack", "defend", "forfeit", "rank", "leaderboard"]
    ...
    if command in ["gamehelp", "duelhelp"]:
        reply_to_room(room_id, "⚔️ **How to Play: Arsenal Clash** ⚔️\n"
                               ...
                               "**2. Accept:** `!acceptduel`\n"
                               "**3. Attack:** `!attack <emoji>` (Use each 💣🔥🚀💥🔫🔪 once)\n"
                               "**4. Defend:** `!defend <emoji>` (Guess the weapon in 15s)\n"
                               "**Goal:** Get more HITs to win!\n"
...
def on_message(ws, message_str):
    ...
    try:
        ...
        if handler == "login" and data.get("status") == "success":
            ...
            bot_state.room_name_to_id[room_name.lower()] = room_id # Update reverse mapping
            ...
            if rejoin_room_name:
                bot_state.room_name_to_id.pop(rejoin_room_name.lower(), None) # Clean up reverse map

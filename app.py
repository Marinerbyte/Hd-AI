...
from postgrest import APIError as SupabaseAPIError, APIResponse
...
# --- NEW AUTO IMAGE MANAGER CLASS ---
class AutoImageManager:
    def __init__(self, bot_state_ref):
        self.bot_state = bot_state_ref
        self.active_threads = {}
        logging.info("AutoImageManager initialized.")

    def _poster_loop(self, room_name, interval_minutes):
        time.sleep(10) # Initial delay to ensure bot has joined the room
        room_id = self.bot_state.room_name_to_id.get(room_name.lower())
        if not room_id:
            logging.error(f"[AutoImg] Cannot start for room '{room_name}', Room ID not found.")
            return

        logging.info(f"[AutoImg] Starting poster loop for room '{room_name}' with {interval_minutes} min interval.")
        interval_seconds = interval_minutes * 60

        while not self.bot_state.stop_bot_event.is_set():
            try:
                # Wait for the interval
                self.bot_state.stop_bot_event.wait(interval_seconds)
                if self.bot_state.stop_bot_event.is_set(): break

                # Check if the task is still active in DB
                if supabase:
                    config_res = supabase.table('auto_image_config').select('is_active').eq('room_name', room_name).maybe_single().execute()
                    if not config_res.data or not config_res.data.get('is_active'):
                        logging.info(f"[AutoImg] Stopping task for '{room_name}' as it's no longer active in DB.")
                        break

                    # Get all image links
                    links_res = supabase.table('image_links').select('url').execute()
                    if not links_res.data:
                        logging.warning(f"[AutoImg] No image links found in DB for room '{room_name}'. Skipping post.")
                        continue

                    # Pick a random link and post it
                    random_link = random.choice(links_res.data)['url']
                    reply_to_room(room_id, random_link)
                    logging.info(f"[AutoImg] Posted random image to '{room_name}'.")
                else:
                    logging.warning("[AutoImg] Supabase not available. Cannot post image.")
                    break # Stop loop if supabase is down

            except Exception as e:
                logging.error(f"[AutoImg] Error in poster loop for '{room_name}': {e}", exc_info=True)
                time.sleep(60) # Wait a minute before retrying on error

        if room_name in self.active_threads:
            del self.active_threads[room_name]
        logging.info(f"[AutoImg] Poster loop for '{room_name}' has ended.")

    def start_task(self, room_name, interval_minutes):
        if room_name in self.active_threads:
            logging.warning(f"[AutoImg] Task for '{room_name}' is already running. It will be restarted.")

        thread = threading.Thread(target=self._poster_loop, args=(room_name, interval_minutes), daemon=True)
        self.active_threads[room_name] = thread
        thread.start()

    def load_and_start_all_tasks(self):
        logging.info("[AutoImg] Loading and starting all active tasks from DB...")
        try:
            if not supabase:
                logging.warning("[AutoImg] Supabase not available. Cannot load tasks.")
                return
            response = supabase.table('auto_image_config').select('*').eq('is_active', True).execute()
            if response.data:
                for config in response.data:
                    self.start_task(config['room_name'], config['interval_minutes'])
            logging.info(f"[AutoImg] Found and started {len(response.data)} active tasks.")
        except Exception as e:
            logging.error(f"[AutoImg] Failed to load auto image tasks from DB: {e}")
...
if Config.SUPABASE_URL and Config.SUPABASE_KEY:
    try:
        ...
        bot_state.auto_image_manager = AutoImageManager(bot_state)
    except Exception as e:
        logging.critical(f"🔴 FAILED TO INITIALIZE SUPABASE: {e}")
else:
    logging.warning("⚠️ Supabase URL/Key missing. AI and Image features will NOT work.")
...
def initialize_database():
    ...
    try:
        supabase.table('image_links').select('id', head=True).execute()
        logging.info("✅ Table 'image_links' found.")
    except SupabaseAPIError as e:
        if "relation \"public.image_links\" does not exist" in e.message:
            logging.critical("🔴🔴🔴 CRITICAL: Table 'image_links' is MISSING. Please create it in Supabase SQL Editor.")

    try:
        supabase.table('auto_image_config').select('room_name', head=True).execute()
        logging.info("✅ Table 'auto_image_config' found.")
    except SupabaseAPIError as e:
        if "relation \"public.auto_image_config\" does not exist" in e.message:
            logging.critical("🔴🔴🔴 CRITICAL: Table 'auto_image_config' is MISSING. Please create it in Supabase SQL Editor.")
    ...
    try:
        ...
        if "relation \"public.personalities\" does not exist" in e.message:
             logging.critical("🔴🔴🔴 CRITICAL: Core table 'personalities' is MISSING. Please create it in Supabase.")
        else:
            logging.error(f"🔴 Failed to sync personalities. Error: {e.message}")
    except Exception as e:
...
def handle_master_command(sender, command, args, room_id):
    if not supabase:
        return reply_to_room(room_id, "❌ Database is not connected. Cannot perform this action.")
        
    current_room_name = bot_state.room_id_to_name.get(room_id, "unknown")

    try:
        # --- NEW IMAGE COMMANDS ---
        if command == 'addimg':
            if not args: return reply_to_room(room_id, "Usage: `!addimg <image_link>`")
            img_url = args[0]
            supabase.table('image_links').insert({'url': img_url}).execute()
            reply_to_room(room_id, "✅ Image link added.")

        elif command == 'listimg':
            response = supabase.table('image_links').select('id, url').order('id', desc=False).execute()
            if not response.data:
                return reply_to_room(room_id, "🖼️ The image list is empty.")
            
            message = "🖼️ **Image List:**\n"
            for i, item in enumerate(response.data, 1):
                message += f"`{i}.` {item['url']}\n"
            reply_to_room(room_id, message)

        elif command == 'delimg':
            if not args or not args[0].isdigit():
                return reply_to_room(room_id, "Usage: `!delimg <number>` (use `!listimg` to get the number)")
            
            item_number_to_delete = int(args[0])
            response = supabase.table('image_links').select('id').order('id', desc=False).execute()
            
            if not response.data or item_number_to_delete <= 0 or item_number_to_delete > len(response.data):
                return reply_to_room(room_id, f"❌ Invalid number. There are only {len(response.data)} images.")

            item_id_to_delete = response.data[item_number_to_delete - 1]['id']
            supabase.table('image_links').delete().eq('id', item_id_to_delete).execute()
            reply_to_room(room_id, f"✅ Image #{item_number_to_delete} has been deleted.")

        elif command == 'autoon':
            if not args or not args[0].isdigit():
                return reply_to_room(room_id, "Usage: `!autoon <minutes> [room_name]`")
            
            interval = int(args[0])
            target_room = " ".join(args[1:]) if len(args) > 1 else current_room_name
            
            if target_room.lower() not in bot_state.room_name_to_id:
                return reply_to_room(room_id, f"❌ I am not in a room named '{target_room}'.")

            supabase.table('auto_image_config').upsert({
                'room_name': target_room,
                'interval_minutes': interval,
                'is_active': True
            }).execute()
            bot_state.auto_image_manager.start_task(target_room, interval)
            reply_to_room(room_id, f"✅ Auto-posting started in '{target_room}' for every {interval} minutes.")

        elif command == 'autooff':
            target_room = " ".join(args) if args else current_room_name
            supabase.table('auto_image_config').upsert({
                'room_name': target_room,
                'is_active': False
            }).execute()
            reply_to_room(room_id, f"⏹️ Auto-posting stopped for '{target_room}'. It will fully stop on the next cycle.")

        elif command == 'autostatus':
            response = supabase.table('auto_image_config').select('*').execute()
            if not response.data:
                return reply_to_room(room_id, "ℹ️ No rooms are configured for auto-posting.")
            
            message = "📊 **Auto-Posting Status:**\n"
            for config in response.data:
                status = "🟢 Active" if config['is_active'] else "🔴 Inactive"
                message += f"- `{config['room_name']}`: {status} ({config['interval_minutes']} mins)\n"
            reply_to_room(room_id, message)
        ...
        if "duplicate key value violates unique constraint" in e.message:
            reply_to_room(room_id, "🤔 That item seems to exist already.")
        else:
            logging.error(f"Error on master command '{command}': {e}", exc_info=True)
            reply_to_room(room_id, f"My database is acting up. Couldn't do that. 💅 Error: {e.message}")
    except Exception as e:
        logging.error(f"Error on master command '{command}': {e}", exc_info=True)
        reply_to_room(room_id, "Something went wrong. Couldn't do that. 💅")
...
def process_command(sender, room_id, message_text):
    ...
    if command == 'help':
        reply_to_room(room_id, f"💖 **{Config.BOT_USERNAME}'s Commands** 💖\n"
                               ...
                               f"- **Image cmds:** `!addimg`, `!listimg`, `!delimg`, `!autoon`, `!autooff`, `!autostatus`\n"
    ...
    if command == 'j':
        ...
        master_commands = [
            'addimg', 'listimg', 'delimg', 'autoon', 'autooff', 'autostatus',
            'am', 'dm', 'listmasters', 'adb', 'rmb', 'pers', 'addpers', 'delpers', 'listpers'
        ]
...
def on_message(ws, message_str):
    ...
    try:
        ...
        if handler == "login" and data.get("status") == "success":
            ...
            # Start auto-posters after a delay to let rooms join
            def delayed_start():
                time.sleep(10) # 10 second delay
                if bot_state.auto_image_manager:
                    bot_state.auto_image_manager.load_and_start_all_tasks()

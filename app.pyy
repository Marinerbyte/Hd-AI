...
class HowdiesBot:
    def __init__(self, username, password, initial_rooms):
        ...
        # <-- BADLAV YAHAN: Naya signal system
        self.initial_rooms_joined_event = threading.Event()
    ...
    def scan_all_rooms_and_report(self):
        ...
        self.log("INFO: Profile visit process thread shuru ho gaya hai. Rooms join hone ka intezar kar raha hai...")
        
        # <-- BADLAV YAHAN: Ab 15 second nahi, balki signal ka intezar karega
        # Timeout 90 seconds ka hai, agar 90 sec tak rooms join na hue to aage badhega
        is_ready = self.initial_rooms_joined_event.wait(timeout=90)
        
        if not is_ready:
            self.log("CRITICAL: 90 seconds ho gaye, lekin sabhi default rooms join nahi hue. Profile visit process ruka.")
            return

        self.log(f"INFO: Signal mil gaya. Scan reports master '{self.MASTER_USERNAME}' ko DM ki jayengi.")

        with self.room_data_lock:
            rooms_to_scan = list(self.current_rooms.items())
        
        self.log(f"INFO: Profile visiting started. Total rooms to process: {len(rooms_to_scan)}")
        ...
        for room_id, room_data in rooms_to_scan:
            ...
            self.log(f"VISITING: Processing room '{room_name}'. Total users to visit: {len(users_to_visit)}")
            ...
            self.log(f"REPORT: Report for room '{room_name}' has been sent to the master via DM.")
            time.sleep(5) 
            
        self.log("INFO: All rooms have been processed.")
                    ...
                    if handler == "login" and data.get("status") == "success":
                        ...
                        if room_id and name:
                            with self.room_data_lock:
                                ...
                                # <-- BADLAV YAHAN: Signal bhejo jab sab rooms join ho jayein
                                if len(self.current_rooms) >= len(self.INITIAL_ROOMS):
                                    if not self.initial_rooms_joined_event.is_set():
                                        self.log("INFO: Sabhi default rooms join ho gaye. Scanner ko signal bhej raha hu.")
                                        self.initial_rooms_joined_event.set()

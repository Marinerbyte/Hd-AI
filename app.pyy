...
class HowdiesBot:
    def __init__(self, username, password, initial_rooms):
        ...
        # <-- BADLAV YAHAN: Naya retry system ke liye state
        self.rooms_to_join = set(r.lower() for r in self.INITIAL_ROOMS)
        self.join_attempts = {r.lower(): 0 for r in self.INITIAL_ROOMS}
    ...
    def handle_master_command(self, command_message, source_room_id):
        ...
        if command == "!ping":
            ...
            self.log("INFO: Master ne manual profile visit ka command diya hai.")
            self.send_dm_message(self.MASTER_USERNAME, "Okay master, manual profile visit process shuru kar raha hoon...")
            manual_scan_thread = threading.Thread(target=self.scan_all_rooms_and_report, args=(False,))
            manual_scan_thread.daemon = True
    ...
    def visit_user_profile(self, username):
        """Sends the correct payload to visit a user's profile and generate a view."""
        self.log(f"INFO: Visiting profile of user: {username} (to generate view)")
    ...
    def scan_all_rooms_and_report(self):
        ...
        if not rooms_to_scan:
            self.log("WARNING: Koi bhi room join nahi hua hai. Profile visit nahi ho sakti.")
        ...
        for room_data in rooms_to_scan:
            ...
            self.log(f"VISITING: Room '{room_name}' process ho raha hai. Total users to visit: {len(users_to_visit)}")
            ...
            report_message = (
                f"✅ Room '{room_name}' visit complete. "
                f"Total {total_users} users me se {visited_count} profiles successfully visited."
            )
    ...
    # <-- YAHAN SABSE ZAROORI BADLAV KIYA GAYA HAI ---
    def manage_room_joins(self):
        """Manages joining initial rooms with retries, then starts the scan."""
        self.log("INFO: Room join manager shuru ho gaya hai. Sabhi 5 rooms ko join karne ki koshish karega.")
        
        while self.rooms_to_join:
            # Jin rooms ko join karna baaki hai, unki ek copy banao
            pending_rooms = list(self.rooms_to_join)
            
            for room_name in pending_rooms:
                if self.join_attempts[room_name] < self.MAX_JOIN_ATTEMPTS:
                    self.log(f"ATTEMPT: Room '{room_name}' ko join karne ki koshish (Attempt: {self.join_attempts[room_name] + 1})")
                    self.send_payload({"handler": "joinchatroom", "name": room_name, "roomPassword": ""})
                    self.join_attempts[room_name] += 1
                else:
                    # Agar 3 koshish ho gayi hain, to haar maan lo
                    self.log(f"FAIL: Room '{room_name}' 3 koshish ke baad bhi join nahi hua. Ise chhod raha hoon.")
                    self.rooms_to_join.remove(room_name)

            # Response ke liye 20 second intezaar karo
            time.sleep(20)

        self.log("INFO: Room join process poora ho gaya hai.")
        
        # Ab profile visit ka kaam shuru karo
            ...
            try:
                ...
                while self.should_run:
                    ...
                    if handler == "login" and data.get("status") == "success":
                        ...
                        # Ab scanner yahan se nahi, balki manage_room_joins se shuru hoga
                        join_manager_thread = threading.Thread(target=self.manage_room_joins)
                        join_manager_thread.daemon = True
                        join_manager_thread.start()

                    elif handler == "joinchatroom":
                        room_name_lower = (data.get('room') or data.get('name') or "").lower()
                        ...
                        if data.get("success"):
                            ...
                            if room_id and name:
                                # Join successful hua, to list se hata do
                                if name.lower() in self.rooms_to_join:
                                    self.rooms_to_join.remove(name.lower())
                        ...
                        elif room_name_lower in self.join_attempts and self.join_attempts[room_name_lower] >= self.MAX_JOIN_ATTEMPTS:
                            # Agar error aaya hai aur 3 koshish ho chuki hain
                            self.log(f"CONFIRMED FAIL: Room '{room_name_lower}' join nahi ho paaya. Response: {data.get('error')}")
                            if room_name_lower in self.rooms_to_join:
                                self.rooms_to_join.remove(room_name_lower)
